...
pub fn parse_image(icon_path: &Path, name: String, code: u64, uid: String) -> Option<ChosenGlyph> {
    ...
    let parser = svg::open(icon_path, &mut content)
        .unwrap_or_else(|error| panic!("SVG icon file \"{}\" could not be read: {error}", icon_path.display()));
    ...
    let mut transforms_map: BTreeMap<i32, String> = BTreeMap::new();

    for event in parser {
        match event {
            svg::parser::Event::Tag(svg::node::element::tag::Group, svg::node::element::tag::Type::Start, attributes) => {
                group_nest_level += 1;
            ...
            svg::parser::Event::Tag(svg::node::element::tag::Group, svg::node::element::tag::Type::End, _) => {
            ...
            svg::parser::Event::Tag(svg::node::element::tag::Path, _, attributes) => {
                if let Some(p) = attributes.get("d") {
                    ...
                    if !transforms.is_empty() {
                        let mut pt = svg_path_ops::pt::PathTransformer::new(current_path);
            ...
            svg::parser::Event::Tag(svg::node::element::tag::SVG, _, attributes) => {
                if let Some(x) = attributes.get("x") {
                    svg_x = x.to_string().parse().expect("couldn't parse \"x\" from svg icon");
                }
                if let Some(y) = attributes.get("y") {
                    svg_y = y.to_string().parse().expect("couldn't parse \"y\" from svg icon");
                }
                if let Some(w) = attributes.get("width") {
                    let w = w.to_string();
                    if w.ends_with("%") {
                        svg_width_percent = w.trim_end_matches("%").parse().expect("couldn't parse \"width\" from svg icon");
                ...
                if let Some(h) = attributes.get("height") {
                    let h = h.to_string();
                    if h.ends_with("%") {
                        svg_height_percent = h.trim_end_matches("%").parse().expect("couldn't parse \"height\" from svg icon");
    ...
    let mut pt = svg_path_ops::pt::PathTransformer::new(svg_path);
    let mut actual_height = svg_height;

    if !svg_view_box.is_empty() {
        let vb = svg_view_box.split(" ").collect::<Vec<_>>();
        if vb.len() != 4 {
            panic!("SVG Icon \"{}\" should have viewBox with 4 elements)", icon_path.display());
        }
        let vb = vb.into_iter().map(|e| e.parse().expect("viewBox element should be a number")).collect::<Vec<f64>>();
        let _x = vb[0];
        let y = vb[1];
        let vb_width = vb[2];
        let vb_height = vb[3];
